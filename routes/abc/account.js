...
router.get("/shipto/:shipToNumber", async (req, res) => {
  try {
    const environment = req.query.env || null;
    const config = getSupplierConfig("abc", environment);
    const token = await getAccessToken(environment);
    const { shipToNumber } = req.params;
    
    if (!shipToNumber) {
      return res.status(400).json({ 
        error: "shipToNumber is required" 
      });
    }
    
    const url = `${config.pricingBase}/api/account/v1/shiptos/${shipToNumber}`;
